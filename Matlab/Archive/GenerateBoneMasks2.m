function [CorticalBoneMask, CorrectedBoneMarrowMask, seeds] = ...
    GenerateBoneMasks2(OneLegVolume, BoneMarrowMask,...
...
%     MuscleMaskCandidates = TissueCandidates .* uint16( MuscleMask );
    ...
    for j=1:nSeeds
        if (zIndex(j) < size(BoneMarrowMask, 3))
...
[bonemarrowSeeds, indexVector] = AddSeeds( BoneMarrowMask );
...
CorrectedBoneMarrowMask = matitk('SCC', [std, rgIterations, 1], single(OneLegVolume), ...
    single([]), bonemarrowSeeds);
CorrectedBoneMarrowMask = uint16(CorrectedBoneMarrowMask) + BoneMarrowMask;
CorrectedBoneMarrowMask = uint16(CorrectedBoneMarrowMask>0);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [seeds, indexVector] = AddSeeds(MaskImage)
I = find(MaskImage);
[xIndex, yIndex, zIndex] = ind2sub(size(MaskImage), I);
seeds = [];
indexVector = [xIndex, yIndex, zIndex];
nSeeds = length(xIndex);
for j=1:nSeeds
    if (zIndex(j) < size(MaskImage, 3))
        seeds = [seeds, xIndex(j), yIndex(j), zIndex(j)];
    end
end
end